...
export class DateInputComponent implements OnInit, ControlValueAccessor {
	...
	writeValue(value: [any, any]): void {
		if (Array.isArray(value) && value.length == 2) {
			...
			if (value.every(d => typeof d === 'string' && moment(d as string).isValid())) {
				this.value = value;
			}
			[this.fromDate, this.toDate] = this.value.map(o => moment(o));
			this.displayValue = this.formatRangeAsString();
		} else if (typeof value === 'string' && moment(value).isValid()) {
			this.fromDate = moment(value);
			this.toDate = null;
			this.displayValue = this.fromDate.format(this.options.format);
		} else {
			this.fromDate = null;
			this.toDate = null;
	...
	dateChanged(changedData: IChangedData): void {
		...
		if (this.options.autoApply && (this.toDate || !this.range)) {
	...
	formatRangeAsString(): string {
		...
		if (!this.range && this.fromDate) {
			if (typeof this.fromDate === 'string') {
				this.fromDate = moment(this.fromDate);
			}

			range = this.fromDate.format(this.options.format);
		} else if (this.range && this.fromDate && this.toDate) {
			if (typeof this.fromDate === 'string') {
				this.fromDate = moment(this.fromDate);
			}
	 ...
	 * Emite el valor
	 *
	 * @memberof DateInputComponent
	 */
	emitValue(): void {
		this.displayValue = this.formatRangeAsString();
		const dates = [moment(this.fromDate)];
		if (this.range) {
			dates.push(moment(this.toDate));
		}
		if (dates.every(d => d.isValid())) {
			const value = dates.map(d => d.format('YYYY-MM-DD'));
			this.onChange(this.range ? value : value[0]);
		} else {
			this.onChange(null);
		}
	}
